...
/** Built-in dictionaries. The server is the source of truth; the client just
 *  lists them (GET /api/presets) and loads one (POST /api/presets/:id/load). */
...
export const findPreset = (id: string): Preset | undefined => PRESETS.find((p) => p.id === id)
